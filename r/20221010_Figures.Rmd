...
uniprot.combined <- dir_ls("formatted_data", regexp = "20221010_figure1_uniprot") %>%
  fread()
...
cp.coverage.raw <- dir_ls("formatted_data", regexp = "20221010_figure1_sites") %>%
  fread()
...
#ggsave("plots/20221010/Figure1d.png", height = 6.5, width = 16.5)
...
treatment.df <- dir_ls("formatted_data", regexp = "20221010_figure2_treatment") %>%
  fread()
...
#ggsave("plots/20221010/Backus_Treatment.png", height = 3, width = 4.25)
...
figure.2c.df <- rbind(
  ...
  dir_ls("formatted_data", regexp = "20221010_figure2_fragment_max-R") %>%
    fread() %>%
  ...
  dir_ls("formatted_data", regexp = "20221010_figure2_scout") %>%
    fread() %>%
    filter(R > 4) %>% ## Liganded in fragment lib
...
#ggsave("plots/20221010/AllScoutFragment_RefDist.png", height = 6, width = 10.5)
...
figure.2d.df <- dir_ls("formatted_data", regexp = "20221010_figure2_fragment-all-") %>%
  fread() %>%
...
sites.elaborated.electrophiles <- dir_ls("formatted_data", regexp = "20221010_figure2_fragment-all") %>%
  fread() %>%
...
#ggsave("plots/20221010/DrugDiscovery_Distributions.png", height = 3, width = 5)
...
#ggsave("plots/20221010/TheoreticalCoverage.png", height = 4, width = 6.5)
...
upset.sites <- dir_ls("formatted_data", regexp = "20221010.*sites") %>%
  fread() %>%
...
#png(filename = "plots/20221010/FigureS1a_AllSites.png", height = 3, width = 5, units = "in", res = 300)
...
upset.scout.all <- dir_ls("formatted_data", regexp = "20221010.*scout") %>%
  fread() %>%
  select(protein_id, position, 
        Dataset) %>%
  mutate(R.bin = 1,
...
#png(filename = "plots/20221010/FigureS1b_ScoutSites.png", height = 3, width = 5, units = "in", res = 300)
...
upset.scout.liganded <- dir_ls("formatted_data", regexp = "20221010.*scout") %>%
  fread() %>%
  filter(R > 4) %>%
 select(protein_id, position, 
        Dataset) %>%
...
png(filename = "plots/20221010/FigureS1c_LigandedSites.png", height = 3, width = 5, units = "in", res = 300)
...
dir_ls("formatted_data", regexp = "20221010_figure2_fragment-all") %>% 
  fread() %>% 
  filter(quality > 70, 
         AA == "C", 
