...
rbind(
  all.fragment %>%
    ...
    mutate(R.bin = ifelse(R > 4, 
                          ...
                          no = "Not liganded")) %>% 
    select(R.bin, R, Dataset, Frag.Class, HalfShell),
  all.fragment %>%
    filter(Dataset != "Vinogradova et al., 2020\nisoTOP/TMT-ABPP",
           Frag.Class == "Acrylamide",
           quality > 70) %>%
...
#ggsave("plots/20220905/CLvsAC_Boxplots.png", height = 4.5, width = 7.5)
...
unfold <- rbind(#backus.isotop,
      ...
      #caged.cik4.df,
      caged.cbk1.df) %>%
  select(protein_id, position, Dataset, HalfShell) %>%
  distinct() %>%
  group_by(protein_id, Dataset) %>%
  filter(n() > 1) %>%
  ungroup() %>%
  mutate(Exposed = ifelse(HalfShell < 6, 
                          yes = "Exposed",
                          no = "Buried")) %>%
