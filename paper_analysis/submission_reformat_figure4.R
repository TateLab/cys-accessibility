### Reformat data for analysis - Figure 3 (fragment screening datasets)
...
treatment.invitro.raw <- dir_ls(paste0(path.to.data.folder, "/data/backus_nature_2016"), regexp = "/41586.*xlsx") %>%
...
treatment.insitu.raw <- dir_ls(paste0(path.to.data.folder, "/data/backus_nature_2016"), regexp = "/41586.*xlsx") %>%
...
treatment.nonliganded.raw <- dir_ls(paste0(path.to.data.folder, "/data/backus_nature_2016"), regexp = "/41586.*xlsx") %>%
...
### Backus et al., 2016
scout.backus.nature <- dir_ls(paste0(path.to.data.folder, "/data/backus_nature_2016"), regexp = "/41586.*xlsx") %>%
  readxl::read_excel(sheet = 2) %>%
...
## Vinogradova et al., 2020
fragment.vinogradova.cell <- dir_ls(paste0(path.to.data.folder, "/data/vinogradova_cell_2020"), regexp = "/1-.*S0092867420308230.*xlsx") %>%
  readxl::read_excel(sheet = 7) %>%
  select(2,4,11:23) %>%
  filter(!is.na(`...2`)) %>%
  row_to_names(row_number = 1) %>%
...
### Yang et al., 2022
fragment.yang.jacs <- dir_ls(paste0(path.to.data.folder, "/data/yang_jacs_2022"), regexp = "22/ja") %>%
  readxl::read_excel(sheet = 2, na = "--") %>%
...
### Backus et al., 2016
fragment.backus.nature <- dir_ls(paste0(path.to.data.folder, "/data/backus_nature_2016"), regexp = "/41586.*xlsx") %>%
  readxl::read_excel(sheet = 2) %>%
  select(1,3:88) %>%
  separate(1, into = c("protein_id", "position"), sep = "_") %>%
  filter(!str_detect(position, ",")) %>% ## Check no multiply annotated cys
